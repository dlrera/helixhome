...
test.describe('UI/UX Audit - HelixIntel CMMS', () => {
  ...
  test.describe('1. Visual Design & Branding', () => {
    test('1.1 Landing page matches brand identity', async ({ page }) => {
      ...
      const primaryColorElements = await page.locator(`[style*="${BRAND_COLORS.primary}"], [class*="bg-primary"]`).count();
    ...
    test('1.5 Button variants are consistent', async ({ page }) => {
      ...
      const primaryButtons = page.locator('button[class*="primary"], button:not([class*="outline"]):not([class*="ghost"])');
  ...
  test.describe('2. Accessibility (WCAG 2.1 AA)', () => {
    ...
    test('2.2 All interactive elements are keyboard accessible', async ({ page }) => {
      await page.goto('http://localhost:3000/dashboard');
  ...
  test.describe('3. Responsive Design', () => {
    ...
    for (const viewport of viewports) {
      test(`3.${viewports.indexOf(viewport) + 1} Dashboard responsive at ${viewport.name} (${viewport.width}px)`, async ({ page }) => {
      ...
      test(`3.${viewports.indexOf(viewport) + 3 + 1} Assets page responsive at ${viewport.name}`, async ({ page }) => {
        await page.setViewportSize({ width: viewport.width, height: viewport.height });
        await page.goto('http://localhost:3000/assets');
        await page.waitForLoadState('networkidle');
      ...
      test(`3.${viewports.indexOf(viewport) + 6 + 1} Tasks page responsive at ${viewport.name}`, async ({ page }) => {
        await page.setViewportSize({ width: viewport.width, height: viewport.height });
        await page.goto('http://localhost:3000/tasks');
        await page.waitForLoadState('networkidle');
    ...
    test('3.11 Touch targets are 44x44px minimum on mobile', async ({ page }) => {
      await page.setViewportSize({ width: 375, height: 667 });
      await page.goto('http://localhost:3000/dashboard');
  ...
  test.describe('4. Navigation & User Flows', () => {
    test('4.1 Command palette opens with Cmd+K', async ({ page }) => {
      ...
      await expect(commandPalette.first()).toBeVisible({ timeout: 2000 }).catch(() => {});
    });
  ...
  test.describe('5. Forms & Validation', () => {
    ...
    test('5.2 Required fields are marked', async ({ page }) => {
      ...
      const requiredFields = page.locator('input[required], input[aria-required="true"], label:has-text("*")');
    ...
    test('5.3 Error messages are clear and helpful', async ({ page }) => {
      ...
      const firstError = page.locator('[class*="error"], [role="alert"]').first();
    ...
    test('5.5 Date picker is accessible', async ({ page }) => {
      ...
      const dateInputs = page.locator('input[type="date"], button[aria-label*="date"], button[aria-label*="calendar"]');
    ...
    test('5.7 File upload provides feedback', async ({ page }) => {
      ...
      const fileInput = page.locator('input[type="file"], button:has-text(/photo|upload|image/i)');
  ...
  test.describe('6. Loading States & Feedback', () => {
    ...
    test('6.2 Toast notifications appear on actions', async ({ page }) => {
      ...
      const toastContainer = page.locator('[class*="toast"], [role="status"], [data-sonner-toaster]');
  ...
  test.describe('7. Interactive Components', () => {
    ...
    test('7.3 Tooltips appear on hover', async ({ page }) => {
      ...
      const tooltipTriggers = page.locator('[data-tooltip], [title], button[aria-describedby]');
