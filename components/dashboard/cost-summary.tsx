...
import { colors } from '@/lib/config/charts'

// PERFORMANCE OPTIMIZATION: Dynamically import Recharts chart
const MonthlyTrendChart = dynamic(
  ...
  {
    loading: () => (
      <div className="h-[250px] flex flex-col gap-2 p-4">
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between text-sm">
                  ...
                  <span className="text-muted-foreground">
                    {formatCurrency(data.budgetProgress.spent)} /{' '}
                    {formatCurrency(data.budgetProgress.budget)}
                  </span>
                </div>
                ...
                <Progress
                  ...
                  className={
                    data.budgetProgress.isOverBudget ? 'bg-red-100' : ''
                  }
                />
                ...
                <div className="flex items-center justify-between text-xs">
                  ...
                  {data.budgetProgress.isOverBudget ? (
                    <Badge variant="destructive" className="text-xs">
                      Over Budget by{' '}
                      {formatCurrency(Math.abs(data.budgetProgress.remaining))}
                    </Badge>
                  ) : (
                    <span className="text-muted-foreground">
                    ...
                    >
                      <div className="flex items-center gap-2">
                        <div
                          ...
                          style={{
                            backgroundColor:
                              colors.chart.categories[
                                index % colors.chart.categories.length
                              ],
                          }}
                        />
            ...
            {data.monthOverMonth.length > 0 ? (
              <div className="space-y-4">
                <div className="h-[250px]">
                  <MonthlyTrendChart data={data.monthOverMonth} />
                </div>
                ...
                {data.monthOverMonth.length >= 2 && (
                  <div className="p-4 rounded-lg bg-muted/50">
                    {(() => {
                      const currentMonth =
                        data.monthOverMonth[data.monthOverMonth.length - 1]
                      const previousMonth =
                        data.monthOverMonth[data.monthOverMonth.length - 2]
